class DropBoxController{
	...
	constructor(){
		...
		this.reactSnackBarEl = document.querySelector("#react-snackbar-root");
		this.progressBarEl = this.reactSnackBarEl.querySelector(".mc-progress-bar-fg");
	...
	connectFirebase(){
		  ...
		  const firebaseConfig = {
			  apiKey: "AIzaSyAypI_97FS3sBMbH_nWY5ukMFsMd06kr7I",
			  authDomain: "dropbox-clone-27b0e.firebaseapp.com",
			  databaseURL: "https://dropbox-clone-27b0e.firebaseio.com",
			  projectId: "dropbox-clone-27b0e",
			  storageBucket: "dropbox-clone-27b0e.appspot.com",
			  messagingSenderId: "979228764126",
			  appId: "1:979228764126:web:005f0076c9034beda85f4f"
			};
		  // Initialize Firebase
	...
	removeFiles(ref,fileName){
		let fileRef = firebase.storage().ref(ref).child(fileName);
		return fileRef.delete();		//Retorna uma promessa
	}
			...
			folderRef.on('value',snapshot=>{
				...
				snapshot.forEach(item=>{
					...
					if(data.type === 'folder'){
						...
						this.removeFiles(ref + '/' + fileName,data.name).then(()=>{
							resolve({
								fields:{
									key:data.key
								}
	...
	removeTask(){
		...
		this.listFilesEl.querySelectorAll('.selected').forEach(li=>{
	...
	initEvents(){
		...
		this.btnRename.addEventListener('click',event=>{
			let li = this.listFilesEl.querySelector('.selected');

			let file = JSON.parse(li.dataset.file);

			let newNome = prompt("Renomeie o nome do arquivo: ", file.name);
		...
		this.listFilesEl.addEventListener('selectionChange',e=>{
			...
			switch(this.listFilesEl.querySelectorAll('.selected').length){
	...
	uploadComplete(){
		...
		this.showReactSnackBar(false);
	}

	showReactSnackBar(show = true){
		this.reactSnackBarEl.style.display = (show) ? 'block' : 'none';
	}
		...
		[...files].forEach(file=>{
			promises.push(new Promise((resolve,reject)=>{
				...
				ajax.upload.onprogress = event=>{
					this.uploadProgress(event,file,timeInitial);
				}
	...
	uploadProgress(event,file,timeInitial){
		let loaded = event.loaded;
		let total = event.total;
		let timeSpent = Date.now() - timeInitial;
	...
	readFiles(){
		...
		this.getFirebaseRef().on('value',snapshots=>{
			this.listFilesEl.innerHTML = '';
			snapshots.forEach(snapshotItem=>{

				if(snapshotItem.val().type){
					this.listFilesEl.appendChild(this.getFileView(snapshotItem.val(),snapshotItem.key));
				}
	...
	initEventLi(li){
		...
		li.addEventListener('dblclick',event=>{
			...
			switch(file.type){
				...
				default:
					/*
						Precisa ser revisado: está procurando no localhost
						não no firebase;
					*/
		...
		li.addEventListener('click',event=>{
			...
			if(event.shiftKey){
				...
				if(firstLi){
					...
					let indexes = [firstIndex,lastIndex].sort();
			...
			if(!event.ctrlKey){
				this.listFilesEl.querySelectorAll('.selected')
				.forEach(el=>{
